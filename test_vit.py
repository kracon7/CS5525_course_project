...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='vit_result')
    ...
    # test cycle
    confusion_train = np.zeros((train_dataset.num_classes, train_dataset.num_classes))
    net.eval()
    epoch_loss = []
    for i, sample in enumerate(train_data_loader):
        ...
        class_label = label.detach().cpu().numpy()
        confusion_train[class_label, class_pred] += 1

    # test cycle
    confusion_test = np.zeros((train_dataset.num_classes, train_dataset.num_classes))
    net.eval()
    epoch_loss = []
    for i, sample in enumerate(test_data_loader):
        ...
        epoch_loss.append(loss.item())
        print('[%6d: %6d/%6d] test loss: %f, testing accuracy: %f' % \
                (epoch, i, num_batch_test, loss.item(), test_stat[0]/test_stat[1]))

        # update confusion matrix
        class_label = label.detach().cpu().numpy()
        confusion_test[class_label, class_pred] += 1

    confusion_test = confusion_test / np.sum(confusion_test, axis=1, keepdims=True)
    confusion_train = confusion_train / np.sum(confusion_train, axis=1, keepdims=True)
    # plot confusion matrix
    test_confusion_fig, ax1 = plt.subplots(1,1)
    ax1.imshow(confusion_test)
    test_confusion_fig.savefig('%s/confusion_test.png'% (save_path))
    
    train_confusion_fig, ax2 = plt.subplots(1,1)
    ax2.imshow(confusion_train)
    train_confusion_fig.savefig('%s/confusion_train.png'% (save_path))

    
    loss_fig, ax = plt.subplots(1,1)
    loss_fig.suptitle('Training Curve of ViT')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.plot(loss_train, '#eb8787', label='train')
    ax.plot(loss_test, '#87adeb', label='test')
    ax.set_facecolor('#EAEAF2')
    ax.grid(color='w', b=True, which='major')
    ax.legend(loc='upper right', shadow=True)
    loss_fig.savefig('%s/loss.png'% (save_path))
